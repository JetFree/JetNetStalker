...
def select_target():
    choices = {1: "https://www.rghost.net", 2: "https://www.imgur.com",
               3: "https://www.print.sc"}
    ...
    try:
        ...
        if choice in range(1, 4):
            if is_host_available(choices[choice]):
                ...
                print(f"Host {choices[choice]} is unavailable."
                      f" Please select another target")
...
def select_time():
    try:
        ...
        future_date = datetime.now() + timedelta(hours=hours, minutes=minutes,
                                                 seconds=seconds)
        return future_date
    except ValueError:
...
def print_income_data(target, threads_amount, trigger_type, trigger):
    if trigger_type == "size":
        trigger_str = f"until size is" \
                      f" {Fore.CYAN + str(trigger) + ' megabytes' + Fore.RESET}"
    elif trigger_type == "links":
        trigger_str = f"for {Fore.CYAN + str(trigger) + ' links' + Fore.RESET}"
    else:
        trigger_str = f"until time:" \
                      f" {Fore.CYAN + datetime.strftime(trigger, '%H:%M:%S') + Fore.RESET}"
